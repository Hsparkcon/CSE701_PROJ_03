...
/**
 * @brief
 * M_ESTIMATOR_TALWAR class implements weight function (pure virtual) of REGRESSION_ROBUST with Talwar M-Estimator.
 * M_ESTIMATOR_TALWAR class contains tunning constant for Talwar M-Estimator.
 *
 */
class M_ESTIMATOR_TALWAR : public REGRESSION_ROBUST
{
public:
    /**
     * @brief Constructs ROBUST_REGRESSION object with weight function driven from Talwar M-Estimator.
     *
...
private:
    ...
    const double default_tunning_constant = 2.795;
        ...
        std::vector<double> &weight)
    {
        ...
        for (uint32_t i = 0; i < residual.size(); i++)
        {
            weight[i] = std::abs(r_standardized[i]) < 1 ? std::abs(r_standardized[i]) : 0;
        }
    }
};