...
/**
 * @brief
 * M_ESTIMATOR_LOGISTIC class implements weight function (pure virtual) of REGRESSION_ROBUST with Logistic M-Estimator.
 * M_ESTIMATOR_LOGISTIC class contains tunning constant for Logistic M-Estimator.
 *
 */
class M_ESTIMATOR_LOGISTIC : public REGRESSION_ROBUST
{
public:
    /**
     * @brief Constructs ROBUST_REGRESSION object with weight function driven from Logistic M-Estimator.
     *
...
private:
    ...
    const double default_tunning_constant = 1.205;
        ...
        std::vector<double> &weight)
    {
        ...
        for (uint32_t i = 0; i < residual.size(); i++)
        {
            weight[i] = std::tanh(r_standardized[i]) / r_standardized[i];
        }
    }
};