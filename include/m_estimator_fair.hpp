...
/**
 * @brief
 * M_ESTIMATOR_LOGISTIC class implements weight function (pure virtual) of REGRESSION_ROBUST with Fair M-Estimator.
 * M_ESTIMATOR_LOGISTIC class contains tunning constant for Fair M-Estimator.
 *
 */
class M_ESTIMATOR_FAIR : public REGRESSION_ROBUST
{
public:
    /**
     * @brief Constructs ROBUST_REGRESSION object with weight function driven from Fair M-Estimator.
     *
...
private:
    ...
    const double default_tunning_constant = 1.400;
        ...
        std::vector<double> &weight)
    {
        ...
        for (uint32_t i = 0; i < residual.size(); i++)
        {
            weight[i] = 1 / (1 + std::abs(r_standardized[i]));
        }
    }
};